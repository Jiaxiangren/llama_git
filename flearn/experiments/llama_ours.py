...
class CentralTraining(object):
    ...
    def __init__(self, args, share_percent=0, iid=True, unequal=False, result_dir="central"):
        ...
        if self.args.select_method == "random":
            self.layer_index_list = list(range(32))
    ...
    def train(self):
        ...
        # figure out the trainable parametersfile_name = './layer_index/fl/{}.pkl'.format(self.args.task_name)
        if self.args.sort_type == "ours":
            file_name = './mask/fl/{}/{}_{}_{}.pkl'.format(self.args.task_name, self.args.select_layer_num, self.args.prune_ratio, self.args.mask_epochs)
            if os.path.exists(file_name):
                with open(file_name, 'rb') as file:
                    data = pickle.load(file)
                    self.mask = data
            else:
                dir_name = './mask/fl/{}/'.format(self.args.task_name)
                os.makedirs(dir_name, exist_ok=True)
                tmp_mask = []
