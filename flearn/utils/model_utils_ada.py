...
from models.loralib import RankAllocator
from models.loralib import compute_orth_regu
...
def train_delta_lora(args, train_dataloader, model, trainable_parameters):
    ...
    return copy.deepcopy(model.get_copy_of_transfer_weights(trainable_parameters)), tr_loss / global_step, _


def train_ada_lora(args, train_dataloader, model, trainable_parameters):


    model_config, fl_config = args[0], args[1]
    t_total = len(train_dataloader) // fl_config.gradient_accumulation_steps * fl_config.num_local_train_epochs
    final_warmup = math.ceil(0.1 * t_total)

    rankallocator = RankAllocator(
    model, lora_r=8, target_rank=6,
    init_warmup=0, final_warmup=final_warmup, mask_interval=10, 
    total_step=t_total, beta1=0.85, beta2=0.85, 
)
...
def train_share(args, train_dataloader, model, trainable_parameters):
