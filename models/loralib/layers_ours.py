...
class OursLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.r > 0:
                if lora_mask == None:
                    ...
                    # print("######:{}".format(lora_mask))
                    result += (self.lora_dropout(x) @ self.lora_A.transpose(0, 1) @ self.lora_B.transpose(0, 1)) * self.scaling * lora_mask
            return result
        else:
