...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # Jiaxiang add the prompt to the input
        if self.add_prompt_layer < 1 and (prompt_embeddings is not None):
            
            device = input_ids.device if input_ids is not None else inputs_embeds.device
            position_ids = torch.arange(
                past_key_values_length, seq_length+past_key_values_length+prompt_attention_mask.shape[1], dtype=torch.long, device=device
            )
            position_ids = position_ids.unsqueeze(0).view(-1, seq_length+prompt_attention_mask.shape[1])

            hidden_states = torch.cat([prompt_embeddings, hidden_states], dim=1)
        #     attention_mask = self._prepare_decoder_attention_mask(
        #     attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
        # )
            attention_mask = torch.cat([prompt_attention_mask, attention_mask_cp], dim=-1)
            attention_mask = self._prepare_decoder_attention_mask(
            attention_mask, (batch_size, seq_length+prompt_attention_mask.shape[1]), hidden_states, past_key_values_length
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if (idx + 1) == self.add_prompt_layer and prompt_embeddings is not None:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def generate_prompt_embeddings(self, batch_size=None):
        import numpy as np

        self.prompt_embedding = nn.Embedding(self.num_prompt_tokens, self.config.hidden_size)
        indices = np.random.permutation(range(32000))[:self.num_prompt_tokens]
    ...
    def forward(
        ...
        # Jiaxiang, get the prompt embeddings and prompt ids
        if input_ids is not None and inputs_embeds is not None:
        ...
        prompt_embeddings = self.prompt_embedding(prompt_ids.to(input_ids.device))
...
class LlaMaLMHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(config.hidden_size, config.vocab_size+2)
...
class LlamaForSequenceClassification(LlamaPreTrainedModel):
    ...
    def get_copy_of_transfer_weights(self, transfer_param_name_list):
        state_dict = OrderedDict()
        for name, p in self.named_parameters():
            if name in transfer_param_name_list:
                state_dict[name] = copy.deepcopy(p.data)
        return state_dict
    ...
    def update_trainable_weights_from_dict(self, weights):
        # self.load_state_dict(copy.deepcopy(weights), strict=False)
        with torch.no_grad():
            for name, p in self.named_parameters():
                if name in weights.keys():
                    # print(name)
                    p.copy_(copy.deepcopy(weights[name]))
    
    def update_transfer_weights_from_dict(self, weights):
        # self.load_state_dict(copy.deepcopy(weights), strict=False)
        with torch.no_grad():
            for name, p in self.named_parameters():
                if name in weights.keys():
    ...
    def generate_prompt_embeddings(self, batch_size=None):
        ...
        indices = np.random.permutation(range(32000))[:self.config.num_prompt_tokens]
        init_weight = self.model.embed_tokens.weight[indices]
        self.prompt_embedding._load_from_state_dict({"weight": init_weight}, "", None, True, [], [], "")
